...
pda.get.model.output <- function(settings, run.id, inputs) {
  ...
  for(k in 1:n.input){
    ...
    # change the names back again for the aligning step
    # changing.names <- colnames(model)[colnames(model) %in% match.table$mstmip_name & !colnames(model) %in% c("time")]
    # colnames(model)[colnames(model) %in% changing.names] <- match.table$bety_name[match.table$mstmip_name %in% changing.names]
